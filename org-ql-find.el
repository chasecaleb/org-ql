  ...
  (let ((table (make-hash-table :test #'equal))
    ...
    (cl-labels ((action
      ...
      (let* ((completion-styles '(org-ql-find))
             ...
             (selected (completing-read "Definition: " #'collection nil)))
        (goto-char (gethash selected table))
                     ...
                     (completing-read-multiple
                      ...
                      (mapcar #'buffer-name
                              (cl-remove-if-not (lambda (buffer)
                                                ...
                                                (buffer-list))) nil t))
           (current-buffer))))
  (org-ql-find buffers-files :query-prefix "heading:"))
                     ...
                     (completing-read-multiple
                      ...
                      (mapcar #'buffer-name
                              (cl-remove-if-not (lambda (buffer)
                                                ...
                                                (buffer-list))) nil t))
           (current-buffer))))
  (org-ql-find buffers-files :query-prefix "outline-path:"))
